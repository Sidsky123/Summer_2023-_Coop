# -*- coding: utf-8 -*-
"""
Created on Tue Jun 13 13:53:24 2023
...
utils.set_initial_params(dp_clf)
...
class ClientLogR(fl.client.NumPyClient):
    def get_parameters(self, config):  # type: ignore
        ...
        return utils.get_model_parameters(dp_clf)

    def fit(self, parameters, config):  # type: ignore
        utils.set_model_params(dp_clf, parameters)
        with warnings.catch_warnings():
            
            model.fit(X_train, y_train)
        print(f"Training finished for round {config['server_round']}")
       
        return utils.get_model_parameters(dp_clf), len(X_train), {}
...
fl.client.start_numpy_client(server_address="localhost:5000", client= ClientLogR())
